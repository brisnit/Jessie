...
interface AppState {
  ...
  askJessie: (question?: string) => void;
  closeAssistant: () => void;
...
function cannedReply(thread: Thread, from: Role, text: string): string {
  ...
  if (/thesis|essay|draft/.test(t)) return `Happy to help, ${student.first}. Start with the Thesis Development review in Module 6, then bring your working thesis to office hours Thursday (2–4 PM).`;
...
export function AppStateProvider({ children }: { children: ReactNode }) {
  ...
  const [density, setDensityState] = useState<Record<Role, Density>>(() =>
    readJson('jessie:density', { professor: 'comfortable', student: 'comfortable' }));

  useEffect(() => {
    try {
      localStorage.setItem('jessie:density', JSON.stringify(density));
    } catch {
