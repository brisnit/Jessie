...
/**
 * A rule-based stand-in for Jessie's language layer. Every answer is grounded in the same
...
function findCourse(q: string, courseIds: string[]) {
  return courseIds.map((id) => getCourse(id)!).find((c) => {
    ...
    return q.includes(num) || q.includes(c.title.toLowerCase()) || new RegExp(`\\b${dept}\\b`).test(q)
      || (dept === 'eng' && /english|composition|writing/.test(q))
      || (dept === 'psy' && /psych/.test(q)) || (dept === 'bib' && /bibl|bible/.test(q)) || (dept === 'bus' && /business|leadership|marketing/.test(q));
  });
}
...
function professorAnswer(q: string, ctx: AssistantContext): Answer {
  ...
  if (enrollment && /what should|what do|recommend|intervene|help|do for/.test(q)) {
    ...
    return {
      text: `For ${s.first} in ${getCourse(enrollment.courseId)!.code}, Jessie recommends:`,
  ...
  if (enrollment) {
    ...
    return {
      text: `${s.first} is ${riskLabel(enrollment.risk).toLowerCase()} in ${course.code}. Their grade is ${enrollment.current}% today, but Jessie predicts ${enrollment.predicted}% by the end of term if nothing changes. The main signals:`,
  ...
  if (/chang|this week|new|different|lately/.test(q)) {
    ...
    return {
      ...
      footer: 'The largest shift is ENG 101 engagement — mostly concentrated in students struggling with Thesis Development.',
    };
  }
  ...
  return {
    text: 'I can explain what Jessie sees in your Canvas data. Try asking:',
    bullets: ['“Who needs my attention today?”', '“Why is Maya at risk?”', '“How is PSY 210 doing?”', '“Are my interventions working?”'],
  };
}
...
function studentAnswer(q: string): Answer {
  ...
  if (/tonight|today|now|evening|hour|time/.test(q)) {
    ...
    return {
      ...
      bullets: [
        ...
        focus ? '20 min — Draft your thesis and three supporting reasons for the Essay Draft' : '20 min — Get ahead on your next assignment',
  ...
  if (/why|english|eng|trending|down|drop|writing/.test(q) && focus) {
    ...
    return {
      ...
      footer: `The good news: your grade is still ${focus.current}%, and your Research skills are strong (${focus.mastery.find((m) => m.concept === 'Research')?.value ?? 89}%). Catching up this week puts you right back on track.`,
    };
  }
