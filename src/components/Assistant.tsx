...
export function Assistant({ role, ctx }: { role: Role; ctx: AssistantContext }) {
  const { assistant, askJessie, closeAssistant, chat } = useApp();
  ...
  if (!assistant.open) {
    if (chat) return null;
    return (
      <button className="jessie-launcher" onClick={() => askJessie()}>
        <Sparkles size={16} /> Ask Jessie
      </button>
    );
  }
  ...
  return (
    <aside className="drawer assistant" aria-label="Ask Jessie">
      <header className="drawer-head">
        <span className="assistant-mark"><Sparkles size={18} /></span>
        <div className="drawer-title">
          <strong>Ask Jessie</strong>
