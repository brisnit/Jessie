...
const STEPS: Record<string, [string, string][]> = {
  'Thesis Development': [
    ['Watch: Arguable vs. factual claims', '4 min'],
    ['Practice: Revise three thesis statements', '8 min'],
  ...
  return (
    <div className="modal-scrim" onClick={() => openReview(null)}>
      <div className="modal" role="dialog" aria-modal onClick={(ev) => ev.stopPropagation()}>
        ...
        <h2>{concept}</h2>
        <p className="muted">
          Jessie picked these from your {course.code} modules in Canvas because they target exactly what changed.
