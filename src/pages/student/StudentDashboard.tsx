...
export function StudentDashboard() {
  const { askJessie, openReview, doneTasks, toggleTask, threads, openChat } = useApp();
  ...
  const items: DashItem[] = [
    {
      id: 'insight', title: 'Jessie Insight', span: 8,
      node: (
        <InsightCard
          actions={(
            <>
              ...
              <button className="btn btn-outline-light" onClick={() => askJessie('What can I do tonight?')}>What can I do tonight?</button>
            </>
          )}
        >
  ...
  return (
    <CustomDashboard
      role="student"
      storageKey="jessie:layout:student"
      items={items}
      header={(controls) => (
        <PageHeader
