...
export function CourseView({ courseId }: { courseId: string }) {
  const { askJessie, actions } = useApp();
      ...
      <InsightCard
        ...
        actions={(
          <>
            ...
            <button className="btn btn-outline-light" onClick={() => askJessie(`How is ${course.code} doing?`)}>Ask Jessie about {course.code}</button>
          </>
        )}
      >
            ...
            return (
              <Link key={e.id} to={`/professor/course/${courseId}/student/${e.studentId}`} className={`card student-card risk-edge-${e.risk}`}>
                ...
                {done && done.kind !== 'dismissed' && (
                  <div className="sc-done"><CheckCircle2 size={15} /> Action taken today — Jessie is measuring impact</div>
                )}
              </Link>
            );
          })}
