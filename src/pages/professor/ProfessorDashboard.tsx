...
export function ProfessorDashboard() {
  const { threads, openChat, askJessie, addedInterventions } = useApp();
  ...
  const items: DashItem[] = [
    {
      id: 'briefing', title: 'Jessie Insight', span: 8,
      node: (
        <InsightCard
          title="Jessie Insight · Today's briefing"
          actions={(
            <>
              <button className="btn btn-light" onClick={() => askJessie('Who needs my attention today?')}>Who needs me today?</button>
              <button className="btn btn-outline-light" onClick={() => askJessie('What changed this week?')}>What changed this week?</button>
            </>
          )}
        >
          <p className="insight-text">
            Your courses are academically healthy overall, but ENG 101 engagement has declined {-eng.engagementChange}% in two weeks.{' '}
      ...
      node: (
        <Card eyebrow="Prove" title="Are interventions working?" action={<Link to="/professor/impact" className="text-link">Impact <ArrowRight size={14} /></Link>}>
          ...
          <div className="impact-story">
            <div className="muted small">{fmtDate(showcase.steps[0].date)} – {fmtDate(showcase.steps.at(-1)!.date)} · ENG 101</div>
            <div><b>Ethan Brooks</b> · Thesis Development {showcase.masteryFrom}% → {showcase.masteryTo}%</div>
  ...
  return (
    <CustomDashboard
      role="professor"
      storageKey="jessie:layout:professor"
      items={items}
      header={(controls) => (
        <PageHeader
