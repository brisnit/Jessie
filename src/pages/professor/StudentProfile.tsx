...
export function StudentProfile({ courseId, studentId }: { courseId: string; studentId: string }) {
  ...
  const measuring = (title: string, text: string): Intervention => ({
    ...
    steps: [
      ...
      { date: addDays(TODAY_ISO, 7), kind: 'pending', text: `Jessie is measuring impact — results expected ${fmtDate(addDays(TODAY_ISO, 7))}` },
    ],
  });
  ...
  const dismiss = (reason: string) => {
    ...
    setDismissing(false);
    toast('Recommendation dismissed. Jessie will use your feedback to improve future suggestions.');
  };
  ...
  return (
    <div className="page">
      ...
      <div className="grid-12">
        <Card className="span-5" eyebrow="Explainable prediction" title={atRisk ? 'Why Jessie flagged this student' : `Why ${s.first} looks on track`}>
          ...
          {action && action.kind !== 'dismissed' && (
            <div className="rec-done">
              ...
              <div>
                ...
                <p className="rec-detail">Jessie will watch {s.first}'s Canvas activity and {rec.concept} mastery over the next 7 days and report whether this changed the trajectory.</p>
          ...
          {action?.kind === 'dismissed' && (
            <div className="rec-done muted-done">
              ...
              <div>
                ...
                <p className="rec-detail">Reason: {action.note}. Jessie will keep monitoring and suggest again if the trajectory worsens.</p>
              </div>
            </div>
          )}
        </Card>
        ...
        <Card className="span-7" eyebrow="Prove" title="Intervention history">
          {history.length === 0 ? (
            <p className="muted">No interventions yet. When you act on a recommendation, Jessie tracks whether it changed {s.first}'s trajectory.</p>
          ) : (
